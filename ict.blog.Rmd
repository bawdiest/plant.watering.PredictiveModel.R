...
# Output --------------------------------------------------------------------
plot(p_weatherData[, c("TimeStp", "tre200bx")], type = "l", col="blue")
...
a_dataSet <- a_dataSet[, !(names(t_dataSet) %in% grep("^x*2", names(a_dataSet), value=TRUE))] #No X's in dataset!!!
 ...
 # Train models --------------------------------------------
 model <- lm(`y` ~ . - `y*3` - `y*3.1` + 1, data = a_dataSet)
 #model <- lm(`x` ~ prestab0Mean +1, data = a_dataSet)

# Output --------
d <- predict(model,a_dataSet)
d <- cbind(d,a_dataSet$y)
d <- as.data.frame(d)
names(d) <- c("Predicted", "Real")
plot(d, pch = 16, cex = 0.8, col = "blue")
abline(lm(a_dataSet$y ~ predict(model)), col="red")
